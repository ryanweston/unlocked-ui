declare const _default: {
    BaseButton: import("vue").DefineComponent<{
        child: typeof import("vue").VueElement;
        onFocus: ObjectConstructor;
        onClick: FunctionConstructor;
    }, unknown, unknown, {}, {}, import("vue").ComponentOptionsMixin, import("vue").ComponentOptionsMixin, Record<string, any>, string, import("vue").VNodeProps & import("vue").AllowedComponentProps & import("vue").ComponentCustomProps, Readonly<import("vue").ExtractPropTypes<{
        child: typeof import("vue").VueElement;
        onFocus: ObjectConstructor;
        onClick: FunctionConstructor;
    }>>, {}>;
    PrimaryButton: import("vue").DefineComponent<{
        callback: FunctionConstructor;
        contactsPromise: PromiseConstructor;
    }, unknown, unknown, {}, {}, import("vue").ComponentOptionsMixin, import("vue").DefineComponent<{
        child: typeof import("vue").VueElement;
        onFocus: ObjectConstructor;
        onClick: FunctionConstructor;
    }, unknown, unknown, {}, {}, import("vue").ComponentOptionsMixin, import("vue").ComponentOptionsMixin, Record<string, any>, string, import("vue").VNodeProps & import("vue").AllowedComponentProps & import("vue").ComponentCustomProps, Readonly<import("vue").ExtractPropTypes<{
        child: typeof import("vue").VueElement;
        onFocus: ObjectConstructor;
        onClick: FunctionConstructor;
    }>>, {}>, Record<string, any>, string, import("vue").VNodeProps & import("vue").AllowedComponentProps & import("vue").ComponentCustomProps, Readonly<import("vue").ExtractPropTypes<{
        callback: FunctionConstructor;
        contactsPromise: PromiseConstructor;
    }>>, {}>;
    Test: import("vue").DefineComponent<{
        callback: FunctionConstructor;
        contactsPromise: PromiseConstructor;
    }, unknown, unknown, {}, {}, import("vue").ComponentOptionsMixin, import("vue").ComponentOptionsMixin, Record<string, any>, string, import("vue").VNodeProps & import("vue").AllowedComponentProps & import("vue").ComponentCustomProps, Readonly<import("vue").ExtractPropTypes<{
        callback: FunctionConstructor;
        contactsPromise: PromiseConstructor;
    }>>, {}>;
};
export default _default;
